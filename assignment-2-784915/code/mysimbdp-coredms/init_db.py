...
def create_keyspace(session, client_id):
    """
    Init keyspace and create table.
    """
    # Create keyspace for particular client
    session.execute("""
    CREATE  KEYSPACE IF NOT EXISTS """ + client_id + """ 
    WITH REPLICATION = {
    ...
    # Create Table apps
    session.execute("""
    CREATE TABLE IF NOT EXISTS """ + client_id + """ .apps (
        id uuid PRIMARY KEY, 
        name text,
        category text,
        rating float,
...
if __name__ == "__main__":
    ...
    # Demo users
    client_ids = ['dir1', 'dir2']

    # Create keyspaces for each client
    for client in client_ids:
        create_keyspace(session, client)
